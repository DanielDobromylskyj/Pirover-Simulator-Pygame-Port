...
class BasicSprite:
    ...
    def centralray_incidence_point(self, ray_hor_angle_deg):
        ...
        if 0 <= alpha_norm <= theta:
            x_norm += w
            y_norm -= math.tan(theta_radians) * w
        elif theta < alpha_norm <= 90:
            x_norm += math.tan(gamma_radians) * h
            y_norm -= h
        elif 90 < alpha_norm <= 90 + gamma:
            x_norm -= math.tan(gamma_radians) * h
            y_norm -= h
        elif 90 + gamma < alpha_norm <= 180:
            x_norm -= w
            y_norm -= math.tan(theta_radians) * w
        elif 180 < alpha_norm <= 180 + theta:
            x_norm -= w
            y_norm += math.tan(theta_radians) * w
        elif 180 + theta < alpha_norm <= 270:
            x_norm -= math.tan(gamma_radians) * h
            y_norm += h
        elif 270 < alpha_norm <= 270 + gamma:
            x_norm += math.tan(gamma_radians) * h
            y_norm += h
        elif 270 + gamma < alpha_norm <= 360:
            x_norm += w
            y_norm += math.tan(theta_radians) * w
    ...
    def rotate_deg(self, x, y, angle_deg) -> tuple:
    ...
    @staticmethod
    def positive_angle_degrees(angle_deg):
    ...
    @staticmethod
    def positive_angle_radians(angle_rad):
        """ Converts a negative angle to a positive one between 0 and 360 """
...
class SwitchSprite(BasicSprite):
    def __init__(self, texture, x, y, batch, subgroup, parent_window, object_type="switch", idx=-1, window_width=800,
                 window_height=600, mouse_press_handler=None):
        super(SwitchSprite, self).__init__(texture, x, y, batch, subgroup, object_type="switch", idx=-1,
                                           window_width=800,
                                           window_height=600)
    ...
    def on_mouse_press(self, x, y, button, modifiers):
        ...
        if button == 1:  # Was the button click a left click?
            dsq = (self.x - x) ** 2 + (self.y - y) ** 2   # Was the button click on the button?
            # print dsq
            if dsq < self.min_rad_sq:
                if self.mouse_press_handler is not None:  # Is this button used for menu button (mouse_press_handler is not none) or ON/OFF switch
