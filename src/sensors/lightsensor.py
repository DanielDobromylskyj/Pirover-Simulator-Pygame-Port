...
class FixedLightSensor(base_sensor.Sensor):
    ...
    def update_sensor(self, simulator):
        ...
        if light_source is not None and self.parent_robot.receiving_light_focus == True:
            ...
            self.value = MAX_LIGHT_INTENSITY * math.fabs(
                self.gaussian_probability(ang_dist, LIGHT_INTENSITY_MEAN_ANGLE, LIGHT_INTENSITY_STDDEV_ANGLE))
    ...
    def angular_distance(self, light_source, simulator):
        ...
        if alpha_delta_x == 0:
            ...
            alpha = math.atan2(float(light_source.y - robot.y),
                               float(alpha_delta_x))  # atan2 computes the angle within the actual quadrant
        ...
        if is_ray_being_dragged == True:
            ...
            if gamma_delta_x == 0:
                ...
                gamma = math.atan2(float(light_source.y - y_ray_end),
                                   float(gamma_delta_x))  # atan2 computes the angle within the actual quadrant
            ...
            if alpha_delta_x == 0:
                ...
                alpha = math.atan2(float(rot_light_source_y - robot.y),
                                   float(alpha_delta_x))  # atan2 computes the angle within the actual quadrant
    ...
    def angdistance_to_rayend(self, simulator):
        ...
        if alpha < 0:
            alpha += math.pi  # ensuring alpha is between 0 and 180 degrees
        ...
        if delta < 0:
            delta += math.pi  # ensuring delta is between 0 and 180 degrees
    ...
    def normalise_value(self, angdist_to_rayend, simulator):
        ...
        if self.angdist_to_rayend < angdist_to_rayend:  # the ray is further away now from sensor
            self.value -= (self.value * (angdist_to_rayend - self.angdist_to_rayend) / (
                    MAX_VALUED_DIST_TO_RAYEND - self.angdist_to_rayend))

        elif self.angdist_to_rayend > angdist_to_rayend:  # the ray is nearer now from the sensor
            self.value += ((MAX_LIGHT_INTENSITY - self.value) * (self.angdist_to_rayend - angdist_to_rayend) / (
                self.angdist_to_rayend))
